...
//	uint32_t pclk;
  ...
  PWM_Init(1, PWM_MODE_TIMER, (void *) &PWMCfgDat);/* PWM1，使用定时模式（另外还可以使用计数模式） */		
	PINSEL_ConfigPin(1,18,2);/*配置PWM引脚 P1.18: PWM1_1*/
	PWM_ChannelConfig(1, 1, PWM_CHANNEL_SINGLE_EDGE);	/* 配置PWM1通道1输出，单边沿控制模式 */

  pclk = CLKPWR_GetCLK(CLKPWR_CLKTYPE_PER);	/* 配置PWM1通道1，控制周期 */
  ...
  PWM_MatchUpdate(1, 1, (pclk/_BACK_LIGHT_BASE_CLK)/2, PWM_MATCH_UPDATE_NOW);	/* 配置PWM1通道2，控制占空比, 这里设为50% */
  PWMMatchCfgDat.IntOnMatch = DISABLE;
  ...
  PWM_ChannelCmd(1, 1, ENABLE);  /* 使能PWM1通道2输出 */
...
{ 
  PWM_MATCHCFG_Type PWMMatchCfgDat;
  PWM_MatchUpdate(1, 1, level, PWM_MATCH_UPDATE_NOW);	/* 配置PWM1通道2，控制占空比 */
  PWMMatchCfgDat.IntOnMatch = DISABLE;
  PWMMatchCfgDat.MatchChannel = 1;
  PWMMatchCfgDat.ResetOnMatch = DISABLE;
  PWMMatchCfgDat.StopOnMatch = DISABLE;
